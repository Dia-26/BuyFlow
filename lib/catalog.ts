...
export function deduplicateProducts<T extends { title?: string; normalizedTitle?: string }>(items: T[]): T[] {
  const seen = new Set<string>();
  return items.filter(item => {
    const raw = item.title || item.normalizedTitle || "";
    const key = norm(raw);
...
export async function searchCatalog(intent: Intent) {
  ...
  const filteredCandidates = candidates.filter(p => !intent.maxPrice || p.price <= intent.maxPrice);
  const uniqueCandidates = deduplicateProducts(filteredCandidates);
  return rank(uniqueCandidates, intent);
}

export function rank<T extends { price: number; mrp: number; discountPercentage: number; sellerCount: number; stockStatus: string; normalizedTitle: string; title?: string }>(items: T[], intent: Intent): Array<T & { score: number; scoreComponents: { price: number; discount: number; relevance: number; availability: number; sellers: number } }> {
  const uniqueItems = deduplicateProducts(items);
  if (!uniqueItems.length) return [];
  const maxPrice = Math.max(...uniqueItems.map(x => x.price), 1), maxDiscount = Math.max(...uniqueItems.map(x => x.discountPercentage), 1), maxSellers = Math.max(...uniqueItems.map(x => x.sellerCount), 1);
  return uniqueItems.map(p => {
    const components = { price: (maxPrice - p.price) / maxPrice, discount: p.discountPercentage / maxDiscount, relevance: intent.query && p.normalizedTitle.includes(norm(intent.query).split(" ")[0]) ? 1 : 0.7, availability: p.stockStatus === "AVAILABLE" ? 1 : 0, sellers: p.sellerCount / maxSellers };
    ...
    return { ...p, score: Number(score.toFixed(3)), scoreComponents: components };
